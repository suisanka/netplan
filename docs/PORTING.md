...
| Wi-Fi | Runtime-loaded Native Wi-Fi API; `WlanEnumInterfaces` selection, `WLAN_RADIO_STATE` detection, exact `if_index` or radio-on aggregation, current link status, profiles/connect/disconnect, and rollback | GNU target Clippy/build and typed selection/radio tests; physical WLAN scan still required |
